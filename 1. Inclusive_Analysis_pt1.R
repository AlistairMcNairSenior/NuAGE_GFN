...
home_dir<-"/Users/alistairsenior/OneDrive - The University of Sydney (Staff)/Alan Cohen/Analyses"
...
model4<-paste0(model3, " + s(z.comorb)")
...
# Add the micronutrient smoother in to model 3 and create the list
model6<-paste0(model5, " + s(z.comorb)")
...
model8<-paste0(model7, " + s(z.comorb)")
...
#######################################################
######## INTERACTIONS INVOLVING A-Tocopherol ##########
#######################################################

# Which traits do we want to check in the screen (those discussed for model 6)

# Check we are doing the inclusive dataset
if(data_use == "Inclusive" & impute_income == FALSE){

	# Which smoothers involve a-tocopherol and which are significant
	a_toc_smoothers<-grep("z.A_Tocopherol", smoothers)
	a_toc_sig<-adjusted[a_toc_smoothers,]
	...
	names(m)<-paste0("z.", micro_model)
	predict_val<-cbind(predict_val, m)
	predict_val$z.A_Tocopherol<-2
	# Make a prediction for 0 A_Tocopherol as well to estimate the difference
	predict_val0<-predict_val
	predict_val0$z.A_Tocopherol<-0
	
	# Go through each smoother and generate a surface for the trait if the result was significant post adjustment
	for(a in 1:length(a_toc_smoothers)){
		
		# Get the ath a-toc smoother and build in to model 6
		smoother_a<-smoothers[a_toc_smoothers[a]]	
		model_6_a<-paste0("outcome ~ ", smoother_a, " + s(z.alcohol) + s(z.income) + s(z.education) + s(z.age) + s(z.PASE) + sex + smoke + s(sujetno, bs=\"re\") + s(z.comorb)")
		
		# What are the traits we want to model
		traits_a<-names(which(a_toc_sig[a,] == 1))
		data_list_a<-list()
		for(l in 1:length(traits_a)){
			data_list_a[[l]]<-data_list[[match(traits_a, traits)[l]]]
		}
			
		# XYZlist for a
		nuts_a<-word(smoother_a, 2, sep=fixed("("))
		nuts_a<-word(nuts_a, 1, sep=fixed(")"))
		nuts_a<-c(word(nuts_a, 1, sep=fixed(",")), str_trim(word(nuts_a, 2, sep=fixed(",")), side="both"), str_trim(word(nuts_a, 3, sep=fixed(",")), side="both"))
			
		# Re-run the gam function and get a prediction at 2SD Tocopherol
		for(t in 1:length(traits_a)){
			# Get the right data
			data_t<-data_list[[t]]
			data_t$outcome<-data_t[,traits_a[t]]
			
			# Re-run the model
			GAM<-gam(as.formula(model_6_a), data=data_t, method=fit_with)
			
			# Get the predictions for 2-a-toc and 0-a-toc and get the difference if significant otherwise NA
			if(summary(GAM)$s.table[1,4] <= 0.05){
				pred2<-predict(GAM, newdata=predict_val, exclude="s(sujetno)", type="response")
				pred0<-predict(GAM, newdata=predict_val0, exclude="s(sujetno)", type="response")
				pred<-pred2 - pred0
			}else{
				pred<-NA
			}
			...
			# Save the results
			pred_t<-data.frame(trait=traits_a[t], predicted=pred, micro1=nuts_a[1], micro2=nuts_a[2], micro3=nuts_a[3])
			if(t == 1){
				pred_a<-pred_t
			}else{
				pred_a<-rbind(pred_a, pred_t)
			}
		}
		
		# Save the results
		if(a == 1){
			predicted_total<-pred_a
		}else{
			predicted_total<-rbind(predicted_total, pred_a)
		}
	}
	
	# Save the results of the screen
	write.table(predicted_total, file="A_toc_screen.csv", sep=",", row.names=F, col.names=names(predicted_total))
	predicted_total<-read.csv("A_toc_screen.csv")
	
	# Relabel the traits nicely for the plot
	predicted_total$trait<-trait_label[match(predicted_total$trait, traits)]
	
	# Plot a histogram
	pdf("A_toc_screen_hist.pdf", height=5, width=6)
	par(mfrow=c(1,1))
	
	plot<-ggplot(data=predicted_total, aes(predicted, fill=trait)) + 
		geom_histogram(alpha=0.5) +
		xlab("Predicted Effect") +
		ylab("Count") +
		theme_bw() +
		geom_vline(xintercept=0) +
		theme(legend.position=c(0.8, 0.8), legend.background=element_blank(), legend.text=element_text(size=15),
		legend.title=element_blank(), axis.text=element_text(size=19), axis.title=element_text(size=25)) #+
		#scale_fill_manual(values=c("#999999", "#CC79A7", "#56B4E9", "#009E73"))
